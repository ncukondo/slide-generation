...
import { BibliographyGenerator } from '../references/bibliography';
...
export class Pipeline {
  ...
  private async processBibliography(
    ...
    try {
      const result = await this.bibliographyGenerator.generate(citationIds, {
        sort: 'citation-order',
      });
      ...
      const updatedSlides = presentation.slides.map((slide) => {
        if (
          ...
          const sort = (slide.content['sort'] as string) || 'citation-order';
          const sortedResult = this.sortBibliographyEntries(result, sort);

          // Convert CSL items to template-compatible format
          const references = sortedResult.items.map((item) => ({
            id: item.id,
          ...
          return {
            ...
            content: {
              ...
              references,
              _autoGenerated: true,
              _generatedEntries: sortedResult.entries,
            },
          } as ParsedSlide;
        }
        return slide;
  ...
  /**
   * Sort bibliography result by the specified order
   */
  private sortBibliographyEntries(
    result: { entries: string[]; items: CSLItem[]; missing: string[] },
    sort: string
  ): { entries: string[]; items: CSLItem[]; missing: string[] } {
    if (sort === 'citation-order') {
      return result;
    }

    const indexedItems = result.items.map((item, index) => ({
      item,
    ...
    if (sort === 'author') {
      indexedItems.sort((a, b) => {
        const authorA = a.item.author?.[0]?.family || '';
        const authorB = b.item.author?.[0]?.family || '';
        return authorA.localeCompare(authorB);
      });
    } else if (sort === 'year') {
      indexedItems.sort((a, b) => {
        const yearA = this.getYear(a.item) ?? 0;
        const yearB = this.getYear(b.item) ?? 0;
        return yearA - yearB;
      });
    }

    return {
      entries: indexedItems.map((i) => i.entry),
      items: indexedItems.map((i) => i.item),
